    ...
    public class HomePageSteps
    {
        ...
        public void WhenFillInFollowingValues(Table table)
        {
            foreach (var row in table.Rows)
            {
                ...
                email = row[1];
                comment = row[2];
               _home.LeaveComment(row[0], row[1],row[2]);
           }
        ...
        {
            AreEqual(_home.LatestCommentText, comment);
            AreEqual(_home.LatestCommentAuthorText,Concat(name, "\r\n", email));
        }
